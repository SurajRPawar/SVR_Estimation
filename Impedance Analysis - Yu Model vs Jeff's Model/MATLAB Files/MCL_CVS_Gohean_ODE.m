function [xdot,y] = MCL_CVS_Gohean_ODE(t,x,rho,mu,HR,Case,Qvad)
% CVS states:
VLA  = x(1);    
VLV  = x(2);    
P_sa = x(3);  
...
%% Cases: Case = 1 -- > Healthy; Case = 2 --> HF
if Case > 2 || Case < 1
    Case = 1;
end
...
% Left Ventricle
ELV   = [3.25 0.3]; ELVmax = ELV(Case); % maximum LV elastance, mmHg/mL
TLV   = (550 - 1.75*HR)/1000; % LV contraction time, s
delta = 0.666;    % elastance peak percent shift
ALV   = [0.03 0.18]; ALV = ALV(Case); % passive elastance coefficient, mmHg
BLV   = [0.05 0.016]; BLV = BLV(Case); % passive elastance coefficient, 1/mL
...
% Right Ventricle
ERV = [0.75 0.45]; ERVmax = ERV(Case); % maximum RV elastance, mmHg/mL
TRV = TLV;
...
RSA = 0.15;
CSA = [1.25 0.65]; CSA = CSA(Case); % [1.25 0.65]
LSA = 0.0022;
RST = [0.8 0.9]; RST = RST(Case);    % Arterial Tree Resistance
CST = [2 1.5]; CST = CST(Case);      % Arterial Tree Compliance
RSV = [0.025 0.02]; RSV = RSV(Case); % Venous Resistance
CSV = 20; 
