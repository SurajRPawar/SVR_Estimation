% Version 4 - Estimating Rsvr and Cr
...
Vlvhat    = xhat(3);
lambdahat = xhat(4);
x5hat     = xhat(5); % 1/Rsvr*Cao
x6hat     = xhat(6); % 1/Rsvr*Cr

Rsvrhat = 1/(x5hat*Cao);
Crhat   = x5hat*Cao/(x6hat);
...
fx_est = [lambdahat/(150*Lc) - (1/Rsvrhat)*(qaohat/Cao - qrhat/(6*Crhat));
          6*qaohat/(Rsvrhat*Cao) - qrhat*(1/(Crhat*Rm) + 1/(Crhat*Rsvrhat)) + 4*E*Vlvhat/Rm - E*V0s/(10*Rm);
...
%% Symbolic analysis
syms qaohat qrhat Vlvhat V0s lambdahat Rsvrhat Cao Crhat Lc Ra Rm E w Qvad Rc;
x = [qaohat; qrhat; Vlvhat; lambdahat; x5hat; x6hat];
u = [V0s];
